...
// has been loaded which has happened once we are at the Decode phase.
// However, the SUB is a 4 cycle instruction and thus the next state
...
// R3 should now have (4 - 2) = 2
if (cpu.RegFile.reg_file[3] !== 16'h0002) begin
    $display("%d %m: ###ERROR### - Expected Reg-File #3 write = 'h2 Got: %b", $stime, cpu.RegFile.reg_file[3]);
end

$display("%d <-- Difference in R3 is (%d)", $stime, cpu.RegFile.reg_file[3]);
